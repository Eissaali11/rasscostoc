...
export function EnterpriseKpiCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "em-kpi em-ripple em-float enterprise-glass-card rounded-[22px] bg-white border-2 border-[#18B2B0] shadow-card relative overflow-hidden text-rassco-text",
        delay > 0.15 && "em-float-delay-1",
        delay > 0.3 && "em-float-delay-2",
        onClick && "cursor-pointer"
      )}
      onClick={onClick}onMouseDown={onRipple}role={onClick ? "button" : undefined}
    ...
    >
      ...
      <div className="relative z-[1] p-6 space-y-4">
        ...
        <div className="flex items-end justify-between gap-3">
          <AnimatedCounter
            value={value}format={format}className={cn(
