...
export default function VerificationPage() {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: "تم تحديث حالة السيريال",
        description: "تم تعديل حالة الرقم التسلسلي وتحديث سجل العهدة بنجاح",
      });
  ...
  const getCategoryDetails = (category?: string | null) => {
    switch (category) {
      case "devices":
        return { label: "أجهزة POS", icon: Smartphone, color: "text-blue-400 bg-blue-500/10 border-blue-500/20" };
      case "sim":
        return { label: "شريحة اتصال", icon: Handshake, color: "text-purple-400 bg-purple-500/10 border-purple-500/20" };
      case "papers":
        return { label: "ورق طباعة", icon: FileText, color: "text-amber-400 bg-amber-500/10 border-amber-500/20" };
      case "accessories":
        return { label: "ملحقات / شواحن", icon: Cable, color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" };
      default:
        return { label: "صنف آخر", icon: Boxes, color: "text-slate-400 bg-slate-500/10 border-slate-500/20" };
    }
  };
  ...
  const getStatusDetails = (status?: string | null) => {
    switch (status) {
      case "RECEIVED_BY_TECHNICIAN":
        return { label: "في عهدة المندوب", color: "bg-emerald-500/10 text-emerald-400 border-emerald-500/30" };
      case "DELIVERED":
        return { label: "تم تسليمه للعميل", color: "bg-cyan-500/10 text-cyan-400 border-cyan-500/30" };
      case "PENDING_RECEIPT":
        return { label: "قيد توريد الفني", color: "bg-amber-500/10 text-amber-400 border-amber-500/30" };
      case "RETURNED":
        return { label: "مرتجع للمستودع", color: "bg-red-500/10 text-red-400 border-red-500/30" };
      default:
        return { label: status || "غير محدد", color: "bg-slate-500/10 text-slate-400 border-slate-500/30" };
    }
  };
      ...
      <header className="rounded-2xl border border-slate-700/60 bg-slate-900/35 backdrop-blur-xl p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-100 flex items-center gap-2">
            ...
            بوابة التحقق من الرقم التسلسلي
          </h1>
          <p className="text-slate-400 mt-1">
            البحث اللحظي ومراجعة حالة الأجهزة والشرائح ومسؤولية عهدتها ميدانياً
          </p>
        </div>
        ...
        <Card className="bg-slate-900/40 border-slate-700/50 backdrop-blur-xl lg:col-span-1">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">
              قم بمسح الباركود الخاص بالجهاز/الشريحة أو كتابة الرقم التسلسلي يدوياً
            </CardDescription>
          </CardHeader>
            ...
            <form onSubmit={handleSearchSubmit} className="space-y-3">
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder="أدخل الرقم التسلسلي S/N..."
          ...
          <CardFooter className="border-t border-slate-800/60 pt-4 text-xs text-slate-500 flex items-center gap-2">
            ...
            <span>يدعم مسح جميع ملصقات أجهزة نقاط البيع POS وشرائح STC/موبايلي/زين/ليبارا.</span>
          </CardFooter>
        </Card>
          ...
          {isLoading && (
            <Card className="bg-slate-900/40 border-slate-700/50 backdrop-blur-xl h-full flex items-center justify-center p-12">
              <div className="text-center space-y-3">
                ...
                <p className="text-slate-400 text-sm">جاري البحث والتحقق من قواعد البيانات...</p>
              </div>
            </Card>
          )}
          ...
          {!serialQuery && !isLoading && (
            <Card className="bg-slate-900/20 border-dashed border-slate-800 h-full flex items-center justify-center p-12 text-center">
              <div className="max-w-md">
                ...
                <h3 className="text-lg font-bold text-slate-300">بانتظار مسح الرقم التسلسلي</h3>
                <p className="text-slate-500 text-sm mt-1">
                  يرجى توجيه قارئ الباركود أو إدخال الرقم يدوياً في خانة البحث لعرض كافة التفاصيل وحالة العهدة
                </p>
              </div>
            </Card>
          )}
          ...
          {error && !isLoading && (
            <Card className="bg-slate-900/40 border-red-500/20 backdrop-blur-xl h-full flex items-center justify-center p-12 text-center">
              <div className="max-w-md">
                ...
                <h3 className="text-lg font-bold text-slate-100">فشل في جلب البيانات</h3>
                <p className="text-slate-400 text-sm mt-1">{(error as any)?.message || "حدث خطأ غير متوقع"}</p>
              </div>
            </Card>
          )}
            ...
            <Card className="bg-slate-900/40 border-slate-700/60 backdrop-blur-xl overflow-hidden">
              <CardHeader className="bg-gradient-to-r from-slate-950/80 to-slate-900/20 border-b border-slate-800/80 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <CardTitle className="text-slate-100 text-xl font-bold">{itemData.itemTypeName || "صنف غير معروف"}</CardTitle>
              ...
              <CardContent className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-slate-950/30 p-4 rounded-xl border border-slate-800/60 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-slate-500">التصنيف والنوع</p>
                  ...
                  <div className="bg-slate-950/30 p-4 rounded-xl border border-slate-800/60 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-slate-500">تاريخ تسجيل التوريد</p>
                      <p className="text-sm font-bold text-slate-200 mt-0.5">
                  ...
                  <div className="bg-slate-950/30 p-4 rounded-xl border border-slate-800/60 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-slate-500">آخر تحديث للعهدة</p>
                      <p className="text-sm font-bold text-slate-200 mt-0.5">
                ...
                {itemData.itemTypeCategory === "sim" && itemData.carrierName && (
                  <div className="p-4 rounded-xl bg-purple-500/5 border border-purple-500/10 flex items-center justify-between">
                    <span className="text-slate-400 text-sm">مزود خدمة الاتصالات (Carrier Name):</span>
              ...
              <CardFooter className="bg-slate-950/40 border-t border-slate-800 p-6 flex flex-col sm:flex-row gap-3">
                {itemData.status === "RECEIVED_BY_TECHNICIAN" ? (
                  ...
                  >
                    {updateStatusMutation.isPending ? "جاري الحفظ..." : "اعتماد تسليم الجهاز للعميل (إغلاق الطلب)"}
                  </Button>
                  ...
                  >
                    {updateStatusMutation.isPending ? "جاري الحفظ..." : "إعادة العهدة للمندوب"}
                  </Button>
                ) : null}
