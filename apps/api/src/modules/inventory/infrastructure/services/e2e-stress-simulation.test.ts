...
import { items, custodyMovements, itemTypes } from '@shared/schema';
              ...
              where: vi.fn().mockImplementation((condition) => {
                const resultPromise = Promise.resolve(
                  table === itemTypes
                    ? [
                        {
                          id: 'pos-terminal-n950',
                          nameAr: 'أجهزة POS',
                          nameEn: 'POS Terminals',
                          category: 'devices',
                          isActive: true,
                          requiresSerial: true,
                          serialPrefix: 'DEVICESTRESS',
                        },
                      ]
                    : []
                );
  ...
  it('runs a complete lifecycle cycle and stress test with 1,000 operations', async () => {
    ...
    const deliveryPromises = Array.from({ length: 500 }, (_, i) => {
      ...
      const itemRecord = Object.values(dbStore).find(
        (x: any) => x.table === 'items' && x.serialNumber === String(100000 + i)
      );
    ...
    const returnPromises = Array.from({ length: 200 }, (_, i) => {
      ...
      const itemRecord = Object.values(dbStore).find(
        (x: any) => x.table === 'items' && x.serialNumber === String(100500 + i)
      );
