...
import type { CreateItemTypeUseCase } from "@modules/inventory/application/item-types/use-cases/CreateItemType.use-case";
import {
  DuplicateItemTypeNameError,
  ItemTypeIdAlreadyExistsError,
} from "@modules/inventory/domain/item-types/item-type.entity";
import type { InsertItemType } from "@shared/schema";
import type { ItemTypesService } from "@modules/inventory/infrastructure/services/item-types.service";
...
export class ItemTypesController {
  constructor(
    ...
    private readonly createItemTypeUseCase: CreateItemTypeUseCase
  ) {}
  ...
  getAll = asyncHandler(async (req: Request, res: Response) => {
    const types = await this.itemTypesService.getItemTypes();
    res.json(types);
  });
  ...
  getActive = asyncHandler(async (req: Request, res: Response) => {
    const types = await this.itemTypesService.getActiveItemTypes();
    res.json(types);
  });
  ...
  getById = asyncHandler(async (req: Request, res: Response) => {
    const type = await this.itemTypesService.getItemTypeById(req.params.id);
    if (!type) {
  ...
  getSerialTracking = asyncHandler(async (req: Request, res: Response) => {
    ...
    const user = req.user!;
    const rows = await this.itemTypesService.getSerialTrackingByItemType(req.params.id, {
      status,
  ...
  create = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      const type = await this.createItemTypeUseCase.execute(data as InsertItemType);
  ...
  update = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (normalizedData.nameAr || normalizedData.nameEn) {
      const existingTypes = await this.itemTypesService.getItemTypes();
    ...
    let type;
    try {
      type = await this.itemTypesService.updateItemType(req.params.id, normalizedData);
    } catch (error) {
  ...
  toggleActive = asyncHandler(async (req: Request, res: Response) => {
    ...
    let type;
    try {
      type = await this.itemTypesService.updateItemType(req.params.id, { isActive });
    } catch (error) {
      if (error instanceof Error && error.message.toLowerCase().includes("not found")) {
        throw new NotFoundError("Item type not found");
      }
  ...
  toggleVisibility = asyncHandler(async (req: Request, res: Response) => {
    ...
    let type;
    try {
      type = await this.itemTypesService.updateItemType(req.params.id, { isVisible });
    } catch (error) {
      if (error instanceof Error && error.message.toLowerCase().includes("not found")) {
        throw new NotFoundError("Item type not found");
      }
  ...
  delete = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      await this.itemTypesService.updateItemType(req.params.id, { isActive: false });
    } catch (error) {
      if (error instanceof Error && error.message.toLowerCase().includes("not found")) {
        throw new NotFoundError("Item type not found");
      }
