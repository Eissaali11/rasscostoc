...
namespaces.forEach(ns => {
  ...
  if (fs.existsSync(arPath)) {
    try {
      ...
      console.warn(`Error parsing ${arPath}:`, e);
    }
  }
  ...
  if (fs.existsSync(enPath)) {
    try {
      ...
      console.warn(`Error parsing ${enPath}:`, e);
    }
  }
});

// Helper to translate Arabic text to English using offline dictionary
function translateText(arabic) {
  ...
  const englishTokens = tokens.map(token => {
    ...
    if (token.startsWith('ال') && wordMap[token.slice(2)]) {
      return wordMap[token.slice(2)];
    }
    return '';
  }).filter(Boolean);
...
// Identify namespace from file path/content
...
function processLine(line, ns, isInsideBackticks) {
  // If line starts with import or contains it in a standard way, skip
  ...
  // 2. JSX Text Nodes (only if the line contains JSX tags and not inside backticks)
  const hasJsx = !isInsideBackticks && /<[a-zA-Z]|<\/[a-zA-Z]| \/>/.test(codePart);
  if (hasJsx) {
    const jsxTextRegex = /([>}])([^<>{}]*[\u0600-\u06FF]+[^<>{}]*)([<{])/g;
    codePart = codePart.replace(jsxTextRegex, (match, prefix, p1, suffix, offset) => {
      const text = p1.trim();
      if (!text) return match;

      // Ensure we are not inside string quotes
      let inDouble = false;
      let inSingle = false;
  ...
  codePart = codePart.replace(stringLiteralRegex, (match, quote, val) => {
    const text = val.trim();
    if (!text || text.includes('${') || text.includes('t(')) return match;
    if (text === 'محلي' || text === 'سحابي') return match;
    const key = generateKey(text, ns);
    lineModified = true;
    return `t('${ns}.${key}')`;
  });

  // 6. Special inline counts/placeholders: {totalInventory} وحدة
...
allFiles.forEach((filePath) => {
  ...
  const newLines = lines.map((line) => {
    ...
    // Track type or interface declarations
    if (inTypeOrInterface) {
      if (trimmed.endsWith('};') || trimmed.endsWith('}') || (trimmed.includes('=') && trimmed.endsWith(';'))) {
        inTypeOrInterface = false;
      }
      return line;
    }

    if (trimmed.startsWith('interface ') || trimmed.startsWith('type ') || (trimmed.startsWith('export ') && (trimmed.includes('interface ') || trimmed.includes('type ')))) {
      if (!(trimmed.endsWith('};') || trimmed.endsWith('}') || (trimmed.includes('=') && trimmed.endsWith(';')))) {
    ...
    const isInsideBackticks = inBackticks;
    if (backticksCount % 2 !== 0) {
      inBackticks = !inBackticks;
    }

    const res = processLine(line, ns, isInsideBackticks);
    if (res.modified) {
      fileModified = true;
      totalConverted++;
    }
  ...
  if (fileModified) {
    ...
    // Add import statement at the top if useTranslation/useLanguage is not present
    if (!finalContent.includes('useTranslation') && !finalContent.includes('useLanguage')) {
      finalContent = `import { useTranslation } from "@/lib/language";\n` + finalContent;
    }

    // Inject hook
    finalContent = injectTranslationHook(finalContent);
...
console.log(`Done! Refactored ${totalModified} files, converted ${totalConverted} texts to t() dynamic calls.`);
